...
def save_graph(G, output_path, delimiter=',', write_stats=True, write_weights=False, write_dir=True):
    ...
    containing basic graph statistics as provided by the get_stats function. If any node has degree 0 a selfloop edge
    will be written, in order to not lose said node.
    ...
    # Add selfloops for deg 0 nodes
    res = []
    for tpl in list(G.degree):
        if tpl[1] == 0:
            res.append((tpl[0], tpl[0]))
    if len(res) > 0:
    ...
    # Make sure we don't permanently change the input graph
    if len(res) > 0:
...
def read_labels(input_path, delimiter=',', comments='#', idx_mapping=None):
    ...
    labels = np.loadtxt(input_path, delimiter=delimiter, comments=comments, dtype=int)
