...
class EvalSetup(object):
    ...
    def _check_objective(self):
        if self.__getattribute__('objective') == 'robustness' and self.__getattribute__('attack_strategy') is None:
            raise ValueError('For robustness evaluation an attack strategy must be specified!')
    ...
    def _check_task(self):
        ...
        if self.__getattribute__('task') == 'nc':
            ...
            if self.__getattribute__('embtype_other') is None or all(x == 'ne' for x in self.__getattribute__('embtype_other')):
                pass
            else:
    ...
    @property
    def objective(self):
        """Returns a string indicating the objective to evaluate i.e. robustness or performance."""
        return self._config.get('GENERAL', 'objective')

    @property
    def attack_strategy(self):
        """Returns a string indicating the attack strategy to use for robustness evaluation."""
        return self._config.get('GENERAL', 'attack_strategy')

    @property
    def attack_approx(self):
        """Returns a float indicating the fraction of nodes to use for approximating some attack strategies e.g. bc."""
        return self._config.getfloat('GENERAL', 'attack_approx')

    @property
    def budgets(self):
        """Returns a list of values indicating the budgets for the robustness evaluation to evaluate."""
        return self.getlist('GENERAL', 'budgets', float)
    ...
    @property
    def classifier(self):
        """Returns an sklearn classifier used in the selected downstream prediction task."""
        model = self._config.get('GENERAL', 'classifier')
        maximize = self.__getattribute__('maximize')
        if maximize == 'auroc':
            maximize = 'roc_auc'
        if model == 'LogisticRegression':
            return LogisticRegression(solver='liblinear')
        elif model == 'LogisticRegressionCV':
            return LogisticRegressionCV(Cs=10, cv=5, penalty='l2', scoring=maximize, solver='lbfgs', max_iter=100)
        elif model == 'DecisionTreeClassifier':
            return DecisionTreeClassifier()
        elif model == 'SVM':
            parameters = {'C': [0.1, 1, 10, 100, 1000], 'scoring': maximize}
