...
class EvalScores:
    ...
    def __init__(self, cols=None, precatk_vals=None):
        self._cols = cols if cols else list()
        self._precatk_vals = precatk_vals
        self._scoresheet_tr = pd.DataFrame()
        self._scoresheet_te = pd.DataFrame()

    def __getstate__(self):
        d = self.__dict__.copy()
        d["_scoresheet_tr"] = pickle.dumps(d["_scoresheet_tr"])
        d["_scoresheet_te"] = pickle.dumps(d["_scoresheet_te"])
        return d

    def __setstate__(self, d):
        self.__dict__ = d
        self.__dict__["_scoresheet_tr"] = pickle.loads(d["_scoresheet_tr"])
        self.__dict__["_scoresheet_te"] = pickle.loads(d["_scoresheet_te"])

    def log_results(self, results, cols_vals=None):
        # Check that all columns have a col value
        cols_vals = cols_vals if cols_vals else list()
        if len(self._cols) != len(cols_vals):
            raise ValueError('Must supply values for all columns: {}'.format(self._cols))

        if isinstance(results, Results) or isinstance(results, NCResults):
            self._log_result('train', results, cols_vals)
            try:
                self._log_result('test', results, cols_vals)
            except ValueError as e:
                pass
                # raise e
        else:
            for res in results:
                self._log_result('train', res, cols_vals)
                try:
                    self._log_result('test', res, cols_vals)
                except ValueError as e:
                    pass
                    # raise e

    def _log_result(self, tr_te, result, cols_vals):
        # Get col names and values
        metrics, vals = result.get_all(tr_te, self._precatk_vals)
        header = self._cols + ['method', 'eval_time', 'edge_embed_method'] + metrics
        cols_vals = cols_vals + [result.method, result.params.get('eval_time', 'None'),
                                 result.params.get('edge_embed_method', 'None')] + vals

        # Update scoresheets with new results
        if tr_te == 'train':
            if self._scoresheet_tr.empty:
                self._scoresheet_tr = pd.DataFrame(columns=header)

            self._scoresheet_tr.loc[self._scoresheet_tr.shape[0]] = cols_vals

        if tr_te == 'test':
            if self._scoresheet_te.empty:
                self._scoresheet_te = pd.DataFrame(columns=header)

            self._scoresheet_te.loc[self._scoresheet_te.shape[0]] = cols_vals

    def get_pandas_df(self, tr_te='auto'):
        if tr_te == 'train':
            scores = self._scoresheet_tr
        elif tr_te == 'test':
            scores = self._scoresheet_te
        else:
            if self._scoresheet_te.empty:
                scores = self._scoresheet_tr
            else:
    ...
    def write_tabular(self, filename, tr_te='auto', cols='all'):
        header = '\n\nEvaluation results:\n-----------------------\n'
        f = open(filename, 'a')
        f.write(header)
        df = self.get_pandas_df(tr_te)
        if cols != 'all' and not df.empty:
            df_s = df[cols]
        else:
            df_s = df
        df_s.to_csv(f, sep='\t', na_rep='NA')
        f.close()
...
class Results(object):
    ...
    def plot(self, filename=None, results='auto', curve='all'):
        ...
        if curve == 'all' or curve == 'roc':
            ...
            # tn, fp, fn, tp = confusion_matrix(scores.y_true, scores.y_bin).ravel()
            # fpr = np.hstack((fpr, [fp/(fp+tn)]))
            # tpr = np.hstack((tpr, [tp/(tp+fn)]))
    ...
    def save_predictions(self, filename, results='auto'):
        ...
        file. The predictions and corresponding labels are not sorted but randomly shuffled.
...
class NCResults(object):
    ...
    def save(self, filename, results='auto'):
        ...
        f.write("\nParameters: ")
        for k, v in self.params.items():
            if k not in ['train_mrgn', 'test_mrgn', 'train_nids', 'test_nids']:
                f.write(str(k) + ": " + str(v) + ", ")
    ...
    def save_predictions(self, filename, results='auto'):
        """
        Writes the method name, execution parameters, and the train or test predictions for each node to a file.
        ...
        if results == 'train':
            scores = self.train_scores
            ids = self.params['train_nids']
            f.write("\nNodeIDs | Train predictions | Train labels ")
        elif results == 'test':
            if self.test_scores is not None:
                scores = self.test_scores
                ids = self.params['test_nids']
                f.write("\nNodeIDs | Test predictions | Test labels ")
            else:
                raise ValueError('Test scores not initialized!')
        else:
            if self.test_scores is not None:
        ...
        for i in range(len(scores.y_true)):
            f.write("\n{} {} {}".format(ids[i], scores.y_pred[i].item(), scores.y_true[i].item()))
        f.write("\n")
        f.close()

    def save_margins(self, filename, results='auto'):
        """
        Writes the method name, execution parameters, and the train or test prediction margins for each node to a file.
        ...
        filename : string or file
            A file or filename where to store the output.
        results : string, optional
            A string indicating if the 'train' or 'test' prediction margins should be used. Default is 'auto' (selects
            'test' if test prediction margins are logged and 'train' otherwise).
        ...
        # Get the appropriate train or test margins
        if results == 'train':
            scores = self.params['train_mrgn']
            ids = self.params['train_nids']
            f.write("\nNodeID | Train margin ")
        elif results == 'test':
            if self.params['test_mrgn'] is not None:
                scores = self.params['test_mrgn']
                ids = self.params['test_nids']
                f.write("\nNodeID | Test margin ")
            else:
                raise ValueError('Test scores not initialized!')
