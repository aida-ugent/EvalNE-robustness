...
class NCEvaluator(object):
    ...
    def _evaluate_ne(self, X, method_name, node_fracs, shuffles, params=None, train_only=False):
        ...
        for frac in node_fracs:
            for i, sh in enumerate(shuffles):
                ...
                X_test = None
                y_test = None
                if frac < 1.0 and not train_only:
                ...
                train_mrgn, test_mrgn = self.compute_margin(X_train, y_train, X_test, y_test)
                if params is not None:
                    params.update({'train_mrgn': train_mrgn, 'test_mrgn': test_mrgn,
                                   'train_nids': sh[:train_size], 'test_nids': sh[train_size:],
                                   'shuffle': i})
                else:
                    params = {'train_mrgn': train_mrgn, 'test_mrgn': test_mrgn,
                              'train_nids': sh[:train_size], 'test_nids': sh[train_size:],
                              'shuffle': i}
    ...
    def compute_pred(self, X_train, y_train, X_test=None):
        ...
        performs predictions for train and test embeddings. If X_test is None test_pred will be None.
        ...
        X_test : ndarray, optional
            An array containing the test embeddings. Default is None.
    ...
    def compute_margin(self, X_train, y_train, X_test=None, y_test=None):
        """
        Returns the classification margin of train and test samples. If X_test is None test_mrgn will be None.
        ...
        y_test : ndarray, optional
            True test labels. Default is None.

        Returns
        -------
        train_mrgn : ndarray
            The confidence scores for the train data.
        test_mrgn : ndarray
            The confidence scores for the test data. Returns None if X_test is None.

        Notes
        -----
        This function uses the definition of classification margin proposed in
        'Adversarial Attacks on Node Embeddings via Graph Poisoning', Bojchevski and Gunnemann, 2019. For each node
        t the classification margin m(t) = p_{t,c(t)} - max_{c \neq c(t)}(p_{t,c}), where c(t) is the true class of
        node t and p_{t,c(t)} is the probability of the true class. Essentially the margin is the difference between
        the probability of the ground truth class of t and the next most likely class.
        """
        # Map every class to its index in the array of classes
        mapping = dict(zip(self.nc_model.classes_, range(len(self.nc_model.classes_))))

        def func(p, y):
            p = list(p)
            # Some test nodes might have labels unknown to the classifier, for these make the gt prob 0
            indx = mapping.get(y)
            if indx is None:
                gt = 0
            else:
        ...
        train_prob = self.nc_model.predict_proba(X_train)      # probabilities for each node and class
        train_mrgn = map(lambda x: func(x[0], x[1]), zip(train_prob, y_train))

        test_mrgn = None
        if X_test is not None and y_test is not None:
            test_prob = self.nc_model.predict_proba(X_test)
            test_mrgn = map(lambda x: func(x[0], x[1]), zip(test_prob, y_test))

        # Return the predictions
        return train_mrgn, test_mrgn
    ...
    def compute_results(self, method_name, train_pred, train_labels, test_pred=None, test_labels=None, params=None):
        ...
        test_labels : ndarray, optional
            True labels of the test data. Default is None.
        params : dict, optional
            A dictionary of parameters and values to be added to the results class. Default is None.
